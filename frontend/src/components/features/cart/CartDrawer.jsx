    ...
    <div className="flex items-center gap-3 py-3 border-b border-brand-gray-mid last:border-0 animate-fade-in">
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.customizationsText && (
          <p className="text-xs text-gray-400 mt-0.5 leading-tight">{item.customizationsText}</p>
        )}
        <p className="text-brand-yellow font-bold text-sm mt-0.5">{formatPrice(item.price)}</p>
      </div>
...
export default function CartDrawer() {
  ...
  const { 
    items, totalItems, totalPrice, rawTotalPrice, comboDiscount, 
    hotdogsCount, friesCount, drinksCount,
    isCartOpen, setIsCartOpen, clearCart, placeOrder, addItem
  } = useCart();
  ...
  const showFriesUpsell = hotdogsCount > 0 && friesCount === 0;
  const showDrinkUpsell = hotdogsCount > 0 && drinksCount === 0;
  const friesUpsellItem = products.find(p => p.id === 16);
  const drinkUpsellItem = products.find(p => p.id === 12);
        ...
        <div className="flex-1 overflow-y-auto px-5">
          {items.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              ...
              {/* Upsell / Cross-sell */}
              {(showFriesUpsell || showDrinkUpsell) && (
                <div className="mt-4 p-4 bg-brand-gray-mid/30 rounded-2xl border border-brand-yellow/10">
                  <p className="text-white font-bold text-sm mb-3">Que tal acompanhar com...</p>
                  <div className="space-y-3">
                    {showFriesUpsell && friesUpsellItem && (
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">🍟</span>
                          <div>
                            <p className="text-sm text-white font-medium">{friesUpsellItem.name}</p>
                            <p className="text-xs text-brand-yellow font-bold">+{formatPrice(friesUpsellItem.price)}</p>
                          </div>
                        </div>
                        <button onClick={() => addItem(friesUpsellItem)} className="text-xs bg-brand-yellow text-brand-black px-3 py-1.5 rounded-lg font-bold hover:bg-white transition-colors">
                          Add
                        </button>
                      </div>
                    )}
                    {showDrinkUpsell && drinkUpsellItem && (
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">🥤</span>
                          <div>
                            <p className="text-sm text-white font-medium">{drinkUpsellItem.name}</p>
                            <p className="text-xs text-brand-yellow font-bold">+{formatPrice(drinkUpsellItem.price)}</p>
                          </div>
                        </div>
                        <button onClick={() => addItem(drinkUpsellItem)} className="text-xs bg-brand-yellow text-brand-black px-3 py-1.5 rounded-lg font-bold hover:bg-white transition-colors">
                          Add
                        </button>
                      </div>
                    )}
            ...
            <div className="space-y-2 text-sm">
              ...
              {comboDiscount > 0 && (
                <div className="flex justify-between text-green-400 font-medium">
                  <span className="flex items-center gap-1">✨ Desconto Combo</span>
